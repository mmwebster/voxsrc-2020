...
# @brief Model baseclass that loads in specific variants
...
class SpeakerNet(nn.Module):
    ...
    def __init__(self, device, max_frames, lr = 0.0001, margin = 1, scale = 1,
        ...
        if optimizer == 'adam':
            ...
            self.__optimizer__ = torch.optim.SGD(self.parameters(),
                    lr = lr, momentum = 0.9, weight_decay=5e-5);
        else:
    ...
    def train_network(self, loader):
        ...
        for data, data_label in loader:
            ...
            sys.stdout.write("Loss %f EER/T1 %2.3f%% - %.2f Hz "%(loss/counter,
                top1/counter, stepsize/telapsed));
    ...
    def evaluateFromListSave(self, listfilename, print_interval=5000,
            feat_dir='', test_path='', num_eval=10):
        ...
        for idx, file in enumerate(setfiles):
            ...
            inp1 = loadWAV(os.path.join(test_path,file), self.__max_frames__,
                           evalmode=True, num_eval=num_eval).to(self.device)
            ...
            if idx % print_interval == 0:
                sys.stdout.write("\rReading %d: %.2f Hz, embed size %d"%(idx,
                    idx/telapsed,ref_feat.size()[1]));
        ...
        for idx, line in enumerate(lines):
            ...
            if feat_dir == '':
                ...
                ref_feat = torch.load(os.path.join(feat_dir,
                    filedict[data[1]])).to(self.device)
                com_feat = torch.load(os.path.join(feat_dir,
                    filedict[data[2]])).to(self.device)

            if self.__test_normalize__:
                ref_feat = F.normalize(ref_feat, p=2, dim=1)
                com_feat = F.normalize(com_feat, p=2, dim=1)

            dist = F.pairwise_distance(ref_feat.unsqueeze(-1).expand(-1,-1,num_eval),
                    com_feat.unsqueeze(-1).expand(-1,-1,num_eval).transpose(0,2)).detach().cpu().numpy();
    ...
    def loadParameters(self, path):
        ...
        for name, param in loaded_state.items():
            ...
            if self_state[name].size() != loaded_state[origname].size():
                print("Wrong parameter length: %s, model: %s,\
                        loaded: %s"%(origname, self_state[name].size(),
